/*
Cho n tam giac trong mat phang Oxy. Mỗi tam giac duoc xac dinh boi 3 diem (x1, y1), (x2, y2), (x3, y3).
Hay tìm dãy tam giác bao nhau có nhiều tam giác nhất.
*/
...
int main()
{
    ...
    for (int i = 1; i < n; i++)
    {
        for (int j = 0; j < i; j++)
        {
            if (contain(a[i], a[j])) // Neu tam giac j nam trong tam giac i
            {
                F[i] = max(F[i], F[j] + 1); 
            }
        }
